...
Source: "$APP_OUT_DIR/redist/ww_icl_redist_intel64_2018.3.210.msi"; DestDir: {tmp}; Flags: deleteafterinstall; AfterInstall: InstallIntel; Check: IntelIsNotInstalled
...
var
  ResultCode: Integer;
begin
  if not Exec( 'msiexec.exe', '/i ' + ExpandConstant('{tmp}\ww_icl_redist_intel64_2018.3.210.msi') + ' /quiet /qn', '', SW_SHOW, ewWaitUntilTerminated, ResultCode) then
  begin
    { you can interact with the user that the installation failed }
